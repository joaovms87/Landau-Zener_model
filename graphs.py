...
for B0 in [-10, -5]:
    ...
    ax2.loglog(taupoints, Wex, 'r.', label=r'$W_\tau(\tau)-W_{ad}(\tau)$')
    ...
    ax6.loglog(taupoints[0::2], Wex[0::2], 'r.', label=r'$W_{\tau,ex}(\tau)$')
    ...
    ax7.loglog(taupoints[0::2], Wex[0::2], 'r.', label=r'$W_{\tau,ex}(\tau)$')
    ax7.loglog(taupoints[1::2], np.real(D2points)[1::2], 'k.', label=r'$D_{\tau,ad}(\tau)$')
    ax7.legend(fancybox=False, edgecolor='black', loc='lower left')
    ax7.set_xlabel(r'$\tau\cdot J$')
    ax7.set_ylabel('$W \cdot J^{-1} \ $ or $\ \ D$')
    ax7.set_title(r'Comparison between $W_{\tau,ex}(\tau)$ and $D_{\tau,ad}(\tau)$')
